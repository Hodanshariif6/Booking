...
function Booking() {
  ...
  const handleBooking = async (room) => {
    ...
    try {
      const res = await axios.post("https://booking-backend-7qa6.onrender.com/create/order", {
        customer: {
          name: customerName,
          email: customerEmail,
          phone: customerPhone,
              ...
              {bookingData.map((room) => (
                ...
                >
                  <td className="py-4 flex items-center gap-4">
                    <img
                      src={`https://booking-backend-7qa6.onrender.com/allImages/${room.prImage?.trim()}`}
                      alt={room.name}
